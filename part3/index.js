...
// otherwise request body not initialized when logger executed 
app.use(requestLogger)  
app.use(cors())
app.use(express.static('dist')) // use static for frontend
...
app.get('/api/notes/:id', (request, response) => {
  ...
  if (note) {
    ...
    response.statusMessage = "Note with the specified ID not found";
...
const generateId = () => {
  const maxId = notes.length > 0
    ? Math.max(...notes.map(n => Number(n.id))) // "three dot" spread syntax transfrom array into indi nums
    : 0
